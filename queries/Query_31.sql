...
create table if not exists public."TaxVariationRequests"
(
    "Id"             integer generated by default as identity
        constraint "PK_TaxVariationRequests"
            primary key,
    "Title"          varchar(256)             not null,
    "Description"    text,
    "RequestedById"  integer                  not null
        constraint "FK_TaxVariationRequests_Users_RequestedById"
            references public."Users"
            on delete cascade,
    "FlightDateFrom" date,
    "FlightDateTo"   date,
...
alter table public."TaxVariationRequests"
    owner to neos;

create index if not exists "IX_TaxVariationRequests_FlightDateFrom"
    on public."TaxVariationRequests" ("FlightDateFrom")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationRequests_FlightDateTo"
    on public."TaxVariationRequests" ("FlightDateTo")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationRequests_IsDeleted"
    on public."TaxVariationRequests" ("IsDeleted")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationRequests_RequestedById"
    on public."TaxVariationRequests" ("RequestedById")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationRequests_Status"
    on public."TaxVariationRequests" ("Status")
    where ("IsDeleted" = false);

create table if not exists public."TaxVariationItems"
(
    "Id"                    integer generated by default as identity
        constraint "PK_TaxVariationItems"
            primary key,
    "TaxVariationRequestId" integer                  not null
        constraint "FK_TaxVariationItems_TaxVariationRequests_TaxVariationRequestId"
            references public."TaxVariationRequests"
            on delete cascade,
    "TaxCode"               varchar(5)               not null,
    "NewValue"              numeric(6, 3)            not null,
...
alter table public."TaxVariationItems"
    owner to neos;

create index if not exists "IX_TaxVariationItems_IsDeleted"
    on public."TaxVariationItems" ("IsDeleted")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationItems_TaxCode"
    on public."TaxVariationItems" ("TaxCode")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationItems_TaxVariationRequestId"
    on public."TaxVariationItems" ("TaxVariationRequestId")
    where ("IsDeleted" = false);

create index if not exists "IX_TaxVariationItems_TaxVariationRequestId_TaxCode"
    on public."TaxVariationItems" ("TaxVariationRequestId", "TaxCode")
    where ("IsDeleted" = false);
...
create table if not exists public."TaxVariationAffectedTaxes"
(
    "Id"                   integer generated by default as identity
        constraint "PK_TaxVariationAffectedTaxes"
            primary key,
    "TaxVariationSeriesId" integer                  not null
        constraint "FK_TaxVariationAffectedTaxes_TaxVariationSeries_TaxVariationSe~"
            references public."TaxVariationSeries"
            on delete cascade,
    "TaxId"                integer                  not null
        constraint "FK_TaxVariationAffectedTaxes_Taxes_TaxId"
            references public."Taxes"
            on delete cascade,
    "TaxVariationItemId"   integer                  not null
        constraint "FK_TaxVariationAffectedTaxes_TaxVariationItems_TaxVariationIte~"
            references public."TaxVariationItems"
            on delete cascade,
    "OriginalValue"        numeric(6, 3)            not null,
    "NewValue"             numeric(6, 3)            not null,
...
alter table public."TaxVariationAffectedTaxes"
    owner to neos;

create index if not exists "IX_TaxVariationAffectedTaxes_Applied"
    on public."TaxVariationAffectedTaxes" ("Applied");

create index if not exists "IX_TaxVariationAffectedTaxes_TaxId"
    on public."TaxVariationAffectedTaxes" ("TaxId");

create index if not exists "IX_TaxVariationAffectedTaxes_TaxVariationItemId"
    on public."TaxVariationAffectedTaxes" ("TaxVariationItemId");

create index if not exists "IX_TaxVariationAffectedTaxes_TaxVariationSeriesId"
    on public."TaxVariationAffectedTaxes" ("TaxVariationSeriesId");

create index if not exists "IX_TaxVariationAffectedTaxes_TaxVariationSeriesId_Applied"
    on public."TaxVariationAffectedTaxes" ("TaxVariationSeriesId", "Applied");

create unique index if not exists "IX_TaxVariationAffectedTaxes_TaxVariationSeriesId_TaxId"
    on public."TaxVariationAffectedTaxes" ("TaxVariationSeriesId", "TaxId");
